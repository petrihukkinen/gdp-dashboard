...
| **material** | No numeric threshold — invites disputes over trivial events. | **Decided by Petri, Cycle 4 (2026-07-11): €100,000 verified value per event OR 0.25% of Annual Contract Value, whichever is higher.** Events below this are ineligible for Event 3/4 payment attribution or Value Validation Board case review. Extension to Event 1 (periodic claims) and to sub-threshold secondary/netted issues is proposed but not yet confirmed — see `gate3_readiness_assessment.md`. | Threshold-setting itself can still be gamed by structuring claims just above/below the line — needs periodic review even with a fixed number. |
